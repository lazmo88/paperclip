...
function useGatewayModels(
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        if (frame.type === "res" && frame.id === "c1") {
          if (!frame.ok) { gotResult = true; setError(frame.error?.message ?? "Connect failed"); setModels([]); setLoading(false); clearTimeout(timer); ws?.close(); return; }
    ...
    let gotResult = false;
    ws.onerror = () => { if (!cancelled) { gotResult = true; setError("WebSocket connection error"); setModels([]); setLoading(false); clearTimeout(timer); } };
    ws.onclose = () => { if (!cancelled && !gotResult) { setError("Connection closed"); setModels([]); setLoading(false); } clearTimeout(timer); };
    return () => { cancelled = true; clearTimeout(timer); ws?.close(); };
  }, [debouncedUrl, token, role, scopes, refreshKey]);
...
function useGatewayAgents(
  ...
  }, [debouncedAgentUrl, token, role, scopes, refreshKey]);
...
function ModelSelector({
  ...
  const { models, loading, error, refresh } = useGatewayModels(wsUrl, token, role, scopes);
...
function AgentSelector({
  ...
  const { agents, defaultId, loading, error, refresh } = useGatewayAgents(wsUrl, token, role, scopes);
  ...
  return (
    <>
      ...
      <AgentSelector
        wsUrl={wsUrl}
        token={wsToken}
        role={String(config.role ?? "operator")}
      ...
      {sessionStrategy === "fixed" && (
        <Field label="Session key">
          {isCreate ? (
            <input
              value={String((values as unknown as Record<string, unknown>)?.openclawSessionKey ?? "paperclip")}
              onChange={(e) => set!({ openclawSessionKey: e.target.value } as Partial<typeof values & { openclawSessionKey: string }>)}
              className={inputClass}
