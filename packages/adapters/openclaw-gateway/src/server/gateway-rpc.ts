...
export async function gatewayRpc<T = unknown>(
  ...
  if (_depth > 1) {
    return { ok: false, error: { code: "RECURSION_LIMIT", message: "Auto-pair retry limit exceeded" } };
  }
      ...
      if (frame.type === "res" && frame.id === connectId) {
        if (!frame.ok) {
          ...
          if (isPairingRequired && authToken && device) {
            // Mark as done BEFORE closing socket to prevent onclose/onerror from resolving the promise
            done = true;
            clearTimeout(timer);
            try { ws.close(); } catch { /* ignore */ }
            // Auto-approve pairing then retry (pairing connection uses token-only auth with operator.pairing scope)
            autoApprovePairing(urlStr, headers, authToken, device, role, scopes)
              .then((pairOk) => {
                if (pairOk) {
                  // Retry the whole RPC call after successful pairing
                  gatewayRpc<T>(config, method, params, timeoutMs, _depth + 1).then((result) => resolve(result));
                } else {
                  resolve({ ok: false, error: { code: "PAIRING_FAILED", message: "Auto-pairing failed. Approve the device manually: openclaw devices approve --latest" } });
                }
              })
              .catch(() => resolve({ ok: false, error: { code: "PAIRING_ERROR", message: "Auto-pairing error. Approve the device manually: openclaw devices approve --latest" } }));
            return;
          }
    ...
    pairWs.on("message", (raw) => {
      ...
      if (frame.type === "res" && frame.id === listId) {
        ...
        const pending = Array.isArray((frame.payload as Record<string, unknown>)?.pending)
          ...
          : [];
        const match = pending.find((e) => String(e.deviceId ?? "") === device.deviceId);
